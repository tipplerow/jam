 ...
 * {@code LinkedHashMap} for record storage.
 *
 * @param <K> the runtime type of the record key.
 *
 * @param <V> the runtime type of the view records.
 */
public class LinkedHashMapView<K, V> extends HashMapView<K, V> {
    /**
     * Creates a new view over an existing {@code LinkedHashMap}.
     *
     * @param map the backing map for record storage.
     */
    protected LinkedHashMapView(LinkedHashMap<K, V> map) {
        super(map);
    }
     ...
     */
    public static <K, V> LinkedHashMapView<K, V> create(Iterable<V> records, Function<V, K> keyFunc) {
        LinkedHashMap<K, V> map = new LinkedHashMap<K, V>();

        for (V record : records)
            map.put(keyFunc.apply(record), record);

        return new LinkedHashMapView<K, V>(map);
    }
}
